...
func (v *validatorGenerator) generateEnumValidator(name string, enumValues []ast.EnumValue) (string, error) {
	if enumValues[0].Type.AsScalar().IsNumeric() {
		return v.generateTFScalarValidator(name, validators.NewInt64Validator(parseEnumValues(enumValues)), nil)
	}
	return v.generateTFScalarValidator(name, validators.NewStringValidator(parseEnumValues(enumValues)), nil)
}
...
func (v *validatorGenerator) generateScalarValidator(name string, scalar ast.ScalarType) (string, error) {
	if !scalar.IsConcrete() && len(scalar.Constraints) == 0 {
		return "", nil
	}
	switch scalar.ScalarKind {
	case ast.KindString, ast.KindBytes, ast.KindNull:
		return v.generateTFScalarValidator(name, validators.NewStringValidator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindBool:
		return v.generateTFScalarValidator(name, validators.NewBoolValidator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindInt32, ast.KindUint32:
		return v.generateTFScalarValidator(name, validators.NewInt32Validator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindInt64, ast.KindUint64:
		return v.generateTFScalarValidator(name, validators.NewInt64Validator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindFloat32:
		return v.generateTFScalarValidator(name, validators.NewFloat32Validator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindFloat64:
		return v.generateTFScalarValidator(name, validators.NewFloat64Validator([]any{scalar.Value}), scalar.Constraints)
	case ast.KindInt8, ast.KindUint8, ast.KindInt16, ast.KindUint16:
		return v.generateTFScalarValidator(name, validators.NewNumberValidator([]any{scalar.Value}), scalar.Constraints)
	}

	return "", nil
}

func (v *validatorGenerator) generateTFScalarValidator(name string, validateFunc validators.ValidatorFuncs, constraints []ast.TypeConstraint) (string, error) {
	...
	if validateFunc.ValuesMatcher() != "" {
		buffer.WriteString(fmt.Sprintf("return \"%s item must be one of '%s'\"\n", name, validateFunc.Values()))
	} else {
		buffer.WriteString(fmt.Sprintf("return \"%s item must be '%s'\"\n", name, formatConstraints(constraints)))
	}
	buffer.WriteString("}\n")
...
func formatConstraints(constraints []ast.TypeConstraint) string {
	var buffer strings.Builder
	for i, c := range constraints {
		if i != 0 && i < len(constraints) {
			buffer.WriteString(" && ")
		}
		if c.Op == ast.MinLengthOp {
			buffer.WriteString(fmt.Sprintf("len(value) < %v", c.Args[0]))
		} else if c.Op == ast.MaxLengthOp {
			buffer.WriteString(fmt.Sprintf("len(value) > %v", c.Args[0]))
		} else {
			buffer.WriteString(fmt.Sprintf("value %s %v", c.Op, c.Args[0]))
		}
	}
