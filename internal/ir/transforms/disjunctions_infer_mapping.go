...
var _ Transform = (*DisjunctionInferMapping)(nil)
...
func (pass *DisjunctionInferMapping) Process(schemas ir.Schemas) (ir.Schemas, error) {
...
func (pass *DisjunctionInferMapping) processDisjunction(_ *Visitor, schema *ir.Schema, def ir.Type) (ir.Type, error) {
	var err error
...
func (pass *DisjunctionInferMapping) ensureDiscriminator(schema *ir.Schema, def ir.Type) (*ir.DisjunctionType, error) {
	var ok bool
...
func (pass *DisjunctionInferMapping) inferDiscriminatorField(schema *ir.Schema, def *ir.DisjunctionType) (string, bool) {
	fieldName := ""
	...
	for _, branch := range def.Branches {
		...
		for _, field := range structType.Fields {
			if (!field.Type.IsConcreteScalar() ||
				field.Type.AsScalar().ScalarKind != ir.KindString) &&
