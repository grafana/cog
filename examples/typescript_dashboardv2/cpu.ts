    ...
    return new PanelBuilder()
        ...
        .data(
            new QueryGroupBuilder()
                .target(new TargetBuilder().query(basicPrometheusQuery(query, "{{ cpu }}")))
        );
};
...
export const loadAverageTimeseries = (): PanelBuilder => {
    return new PanelBuilder()
        ...
        .data(new QueryGroupBuilder().targets([
            new TargetBuilder().query(basicPrometheusQuery(`node_load1{job="integrations/raspberrypi-node", instance="$instance"}`, "1m load average")),
            new TargetBuilder().query(basicPrometheusQuery(`node_load5{job="integrations/raspberrypi-node", instance="$instance"}`, "5m load average")),
            new TargetBuilder().query(basicPrometheusQuery(`node_load15{job="integrations/raspberrypi-node", instance="$instance"}`, "15m load average")),
            new TargetBuilder().query(basicPrometheusQuery(`count(node_cpu_seconds_total{job="integrations/raspberrypi-node", instance="$instance", mode="idle"})`, "logical cores")),
        ]));
};
...
export const cpuTemperatureGauge = (): PanelBuilder => {
    return new PanelBuilder()
        ...
        .data(
            new QueryGroupBuilder()
                .target(new TargetBuilder().query(basicPrometheusQuery(`avg(node_hwmon_temp_celsius{job="integrations/raspberrypi-node", instance="$instance"})`, "")))
        );
};
