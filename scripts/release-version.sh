...
function clone_kind_registry() {
  ...
  git clone --depth 1 https://github.com/grafana/kind-registry.git "${clone_into_dir}"
}
...
function clone_foundation_sdk() {
  ...
  git clone git@github.com:grafana/grafana-foundation-sdk.git "${clone_into_dir}"
}
...
do
  ...
  rm -rf "${FOUNDATION_SDK_PATH:?}/${language}"
  cp -R "./${codegen_output_path}/${language}" "${FOUNDATION_SDK_PATH}"
done
