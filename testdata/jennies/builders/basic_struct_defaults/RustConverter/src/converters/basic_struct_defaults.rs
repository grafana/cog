...
pub fn some_struct_converter(input: &basic_struct_defaults::SomeStruct) -> String {
    let mut calls: Vec<String> =
        vec!["basic_struct_defaults::SomeStructBuilder::new()".to_string()];
    if input.id != 42 {
    ...
    if !input.uid.is_empty() && input.uid != "default-uid" {
        let mut buffer = String::new();
        buffer.push_str("uid(");
