...
import (
	...
	"github.com/grafana/cog/generated/externalpkg"
)
...
func (resource *SomeStruct) UnmarshalJSONStrict(raw []byte) error {
	...
	if fields["fieldBool"] != nil {
		if string(fields["fieldBool"]) != "null" {
			...
			errs = append(errs, cog.MakeBuildErrors("fieldBool", errors.New("required field is null"))...)

		}
		delete(fields, "fieldBool")
	} else {
		errs = append(errs, cog.MakeBuildErrors("fieldBool", errors.New("required field is missing from input"))...)
	}
